...
class Dataset:
    def __init__(self, cam2world_root, img_root, bbx_root, data_root, sequence, split, frame_num, frame_start, use_stereo):
        ...
        self.images_list = sorted(glob(os.path.join(img_root, '*.png')))
        ...
        for idx in self.image_ids:
            ...
            try:
                self.metas[idx-self.start] = self.func((pose, self.images_list[idx], idx))
            except:
                image_path = os.path.join(img_root, '000000'+str(idx)+'.png')
                self.metas[idx-self.start] = self.func((pose, image_path, idx))
        self.bbx_static = {}
    ...
    def func(self, input_tuple):
        ext_pose, image_path, idx = input_tuple
