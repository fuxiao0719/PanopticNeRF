...
class Visualizer:
    ...
    def visualize(self, output, batch):
        ...
        for b in range(b):
            ...
            np.save('{}/img{:04d}_depth.npy'.format(result_dir, img_id),(pred_depth))
            pred_depth = cv2.applyColorMap(cv2.convertScaleAbs(((pred_depth/pred_depth.max()) * 255).astype(np.uint8),alpha=2), cv2.COLORMAP_JET)
            cv2.imwrite('{}/img{:04d}_depth.png'.format(result_dir, img_id), pred_depth)
            _, pred_semantic = output['semantic_map_0'][b].max(1)
            np.save('{}/img{:04d}_pred_semantic.npy'.format(result_dir, img_id), pred_semantic.reshape(h, w, -1).detach().cpu().numpy())
            ...
            cv2.imwrite('{}/img{:04d}_pred_semantic.png'.format(result_dir, img_id), (color.reshape(h, w, 3)[..., ::-1] * 255).astype(np.uint8))
            mix_semantic = cv2.addWeighted((pred_img[..., [2, 1, 0]] * 255).astype(np.uint8), 0.5, (color.reshape(h, w, 3)[..., ::-1] * 255).astype(np.uint8), 0.5, 0)
            cv2.imwrite('{}/img{:04d}_mix_semantic.png'.format(result_dir, img_id), mix_semantic.astype(np.uint8))
            ...
            cv2.imwrite('{}/img{:04d}_pred_panoptic.png'.format(result_dir, img_id), (panoptic_rgb.reshape(h, w, 3)[..., ::-1] * 255).astype(np.uint8))
            cv2.imwrite('{}/img{:04d}_mix_panoptic.png'.format(result_dir, img_id), (mix_panoptic_rgb).astype(np.uint8))
            np.save('{}/img{:04d}_instance2semantic.npy'.format(result_dir, img_id), instance2semantic)
            panoptic_id_map[panoptic_mask] = pred_semantic[panoptic_mask]
            panoptic_id_map[~panoptic_mask] = pred_instance[~panoptic_mask]
            np.save('{}/img{:04d}_panoptic_id_map.npy'.format(result_dir, img_id), panoptic_id_map)