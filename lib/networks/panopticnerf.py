...
class Network(nn.Module):
    ...
    def render_rays(self, batch, rays, intersection, is_fisheye):
        ...
        frame_idx = is_fisheye * (cfg.train_frames*2) + batch['stereo_num'].item()*cfg.train_frames + batch['idx'].item()
        raw, nablas = self.nerf_0(xyz, z_vals/scale_factor[...,None], ray_dir, frame_idx)
        ...
        # model the sky with less sample points
        ...
        if self.training == False or cfg.use_instance_loss == True:
            with torch.no_grad():
                ...
                if is_fisheye == False:
                    ...
                    id2instance = batch['fisheye_id2instance']
                    instance2id = batch['fisheye_instance2id']
        ...
        normals = nablas / (nablas.norm(2, -1, keepdim=True) + 1e-6)
        normal_map = torch.sum(ret_0['weights'][..., None] * normals, dim=-2)
        # transform to local coordinate system
        rot = batch['pose'][0, :3, :3].to(torch.float32).permute(1, 0).contiguous()
        normal_map = rot @ normal_map[0].permute(1, 0)
        normal_map = normal_map.permute(1, 0).contiguous()[None]
        ret_0['nablas'] = nablas
        ret_0['normal_map'] = normal_map
    ...
    def batchify_rays(self, batch, rays, fisheye_rays, intersection, fisheye_intersection):
        all_ret = {}
        chunk = cfg.chunk_size

        for i in range(0, rays.shape[1], chunk):
            ret = self.render_rays(batch, rays[:,i:i+chunk], intersection[:,i:i+chunk], is_fisheye=False)
            if self.training == False:
                del ret['points_semantic_0'] , ret['weights_0'], ret['z_vals_0'], ret['semantic_point_0'] , ret['nablas_0'] , ret['semantic_filter'], ret['one_hot_all_instance']
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
        ...
        for i in range(0, fisheye_rays.shape[1], chunk):
            fisheye_ret = self.render_rays(batch, fisheye_rays[:,i:i+chunk], fisheye_intersection[:,i:i+chunk], is_fisheye=True)
            ret = {}
            for key in fisheye_ret:
                ret['fisheye_'+key] = fisheye_ret[key]
            if self.training == False:
                del ret['fisheye_points_semantic_0'] , ret['fisheye_weights_0'], ret['fisheye_z_vals_0'], ret['fisheye_semantic_point_0']
                del ret['fisheye_nablas_0'] , ret['fisheye_semantic_filter'], ret['fisheye_one_hot_all_instance']
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
                all_ret[k].append(ret[k])
    ...
    def forward(self, batch):
        rays, fisheye_rays, intersection, fisheye_intersection = batch['rays'], batch['fisheye_rays'], batch['intersection'], batch['fisheye_intersection']
        ret = self.batchify_rays(batch, rays, fisheye_rays, intersection, fisheye_intersection)
        return ret
