...
from lib.utils.data_utils import readVariable, loadCalibrationCameraToPose, build_rays, build_fisheye_rays
...
class Dataset:
    def __init__(self, cam2world_root, img_root, bbx_root, data_root, sequence, split, frame_num, frame_start, use_stereo):
        ...
        if self.use_stereo:
           self.cam2world_dict = self.cam2world_dict_03
           self.fisheye_grid = np.load('datasets/KITTI-360/fisheye/grid_fisheye_03.npy')
        else:
           self.cam2world_dict = self.cam2world_dict_02
           self.fisheye_grid = np.load('datasets/KITTI-360/fisheye/grid_fisheye_02.npy')
        self.fisheye_grid = self.fisheye_grid.reshape(1400, 1400, 4)[::int(1/self.ratio),::int(1/self.ratio),:].reshape(-1, 4)
    ...
    def func(self, input_tuple):
        ...
        H, W = int(1400*self.ratio), int(1400*self.ratio)
        rays = build_fisheye_rays(self.fisheye_grid[:,:3], ext_pose)
    ...
    def generate_npy(self, idx, bbx_npy_root):
        annotationId_list = []
        for i in range(idx-120, idx+1):
            filename, _ = os.path.splitext('000000'+str(i)+'.png')
    ...
    def __getitem__(self, index):
        ...
        if self.use_stereo:
            if os.path.exists(os.path.join(bbx_intersection_root,str(index+self.start)+'_03.npz')) == True:
                return 0
        else:
            if os.path.exists(os.path.join(bbx_intersection_root,str(index+self.start)+'_02.npz')) == True:
                return 0
        ...
        if self.use_stereo:
            np.savez(os.path.join(bbx_intersection_root,str(index+self.start)+'_03.npz'), final_depths, final_annotations)
        else:
            np.savez(os.path.join(bbx_intersection_root,str(index+self.start)+'_02.npz'), final_depths, final_annotations)
...
if __name__ == "__main__":
    ...
    sequence = os.path.join('2013_05_28_drive_' + '000' + str(sequence) + '_sync')
    if use_stereo:
        img_root = os.path.join(data_root, sequence, 'image_03/data_rgb')
    else:
        img_root = os.path.join(data_root, sequence, 'image_02/data_rgb')
